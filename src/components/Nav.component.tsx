...
export function NavComponent() {
	const { visitedPages, addVisitedPage, removeVisitedPageIndicator } = useVisitedPagesStore()
		...
		<>
			...
			<nav className="relative z-[100] hidden h-[100px] w-full items-center justify-center border-b border-gray-300 bg-white py-8 md:flex">
				<div className={'absolute top-8 right-8 flex items-center justify-center'}>
					<Link href={'/photos'} className="bg-mae-950 flex h-10 w-10 items-center justify-center rounded-full p-2">
				...
				<div className="mx-auto h-full w-full max-w-xl px-4 md:px-6 xl:max-w-3xl xl:px-8">
					<div className="text-mae-950 flex h-full w-full items-center justify-between text-xs tracking-wider uppercase no-underline xl:text-sm xl:tracking-widest">
						{navLinks.map(({ href, label, isCustomSvg, isNotifActive }) => (
							<Link href={href} key={href} className="relative" onClick={() => handleClick(href)}>
								{isCustomSvg ? (
									<CustomSvgComponent classNames="w-[50px] h-[50px] bg-mae-950" url="/typoLogo.svg" />
								) : (
									<>
										{label}
