...
import type { NavLink } from '@/components/Nav.component'
...
export function NavPhoneComponent({ navLinks }: NavPhoneProps) {
	...
	const countUnvisitedPages = () => {
		return navLinks.reduce((count: number, link) => {
	...
	return (
		<>
			...
			<nav className={`${isMenuOpen ? 'fixed z-50 h-screen w-screen bg-white md:hidden' : 'hidden'}`}>
